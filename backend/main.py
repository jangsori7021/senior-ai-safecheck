...
def app_home():
 with open("static/v43.html","r",encoding="utf-8") as f:
  ...
  new='''<section id="tools" class="screen"><button class="back" onclick="show(\'home\')">⌂ 처음으로</button><h1>✨ 생활 한눈에</h1><div class="box notice"><b>마음 편한 생활마당</b><br>첫 화면과 겹치지 않는 생활 편의 기능을 모았어요.</div><div class="sectionTitle">🚶 밖에 나갈 때</div><div class="grid"><button class="card" onclick="naver(\'근처 맛집\')"><i>🍚</i><b>뭐 먹지?</b><small>가까운 식당 찾아보기</small></button><button class="card" onclick="naver(prompt(\'어디로 갈까요?\')||\'\')"><i>🗺️</i><b>길 찾아줘</b><small>가고 싶은 곳 찾기</small></button><button class="card" onclick="naver(\'근처 화장실\')"><i>🚻</i><b>화장실 어디?</b><small>가까운 화장실 찾기</small></button><button class="card" onclick="naver(\'근처 약국 병원\')"><i>🏥</i><b>병원·약국</b><small>가까운 곳 찾아보기</small></button></div><div class="sectionTitle">🏠 집에서 편하게</div><div class="grid"><button class="card" onclick="quickAsk(\'계산을 도와줘. 내가 숫자와 계산할 내용을 말할게.\')"><i>🧮</i><b>계산 도와줘</b><small>어려운 계산 쉽게 묻기</small></button><button class="card" onclick="quickAsk(\'휴대폰 사용법을 아주 쉽게 알려줘. 무엇이 어려운지 물어봐 줘.\')"><i>📱</i><b>휴대폰 도움</b><small>사용법을 쉽게 물어보기</small></button><button class="card" onclick="quickAsk(\'오늘 해야 할 일을 정리하고 싶어. 하나씩 물어봐 줘.\')"><i>📝</i><b>오늘 할 일</b><small>하나씩 정리해 보기</small></button></div><div class="sectionTitle">☕ 마음 편한 시간</div><div class="grid"><button class="card" onclick="quickAsk(\'지금 마음이 편안해질 수 있도록 짧고 따뜻한 이야기를 해줘.\')"><i>☕</i><b>잠깐 쉬어가기</b><small>짧고 편안한 이야기</small></button><button class="card" onclick="quickAsk(\'기분 전환을 하고 싶어. 내 기분에 맞는 간단한 활동을 추천해줘.\')"><i>🌿</i><b>기분 전환</b><small>가볍게 할 일 추천</small></button></div><div class="two"><button class="btn alt" onclick="goTop()">⬆ 맨 위로</button><button class="btn" onclick="show(\'home\')">⌂ 홈으로</button></div></section>'''
