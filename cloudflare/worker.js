...
:root{--senior-accent:#315b57;--senior-soft:#eef5f2;--senior-warm:#fff8e8}.hero{padding:22px!important}.hero h1{font-size:34px!important;line-height:1.18!important}.hero p{font-size:19px!important;line-height:1.55!important}.askrow{grid-template-columns:1fr!important}.ask{min-height:76px!important;font-size:22px!important}.hero .voice{background:#fff!important;color:#315b57!important}.hero .text{min-height:58px!important;font-size:18px!important}.card{min-height:128px!important;padding:16px!important}.card b{font-size:21px!important}.card small{font-size:15px!important;line-height:1.45!important}.senior-priority{border:3px solid #315b57!important;box-shadow:0 8px 22px rgba(49,91,87,.18)!important}.senior-quick{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:14px 0}.senior-quick button{border:0;border-radius:18px;min-height:84px;padding:12px;background:#fff;font-size:18px;font-weight:900;color:#26302f;box-shadow:0 4px 14px rgba(33,50,47,.10)}.senior-quick button b{display:block;font-size:19px;margin-top:3px}.senior-quick button small{display:block;font-size:13px;font-weight:700;color:#667572;margin-top:4px}.senior-quick .voice-all{grid-column:1/-1;background:#315b57;color:#fff;min-height:90px}.senior-quick .voice-all small{color:#eaf4f0}.senior-daily{background:var(--senior-warm);border:2px solid #ecdcae;border-radius:18px;padding:14px 15px;font-size:17px;line-height:1.55;margin:12px 0}.senior-daily b{font-size:19px}.senior-note{background:var(--senior-soft);border-radius:16px;padding:12px 14px;font-size:16px;line-height:1.5;margin:12px 0}.sectionTitle{font-size:20px!important}.version{font-size:12px!important}
@media(max-width:390px){.hero h1{font-size:31px!important}.senior-quick{grid-template-columns:1fr 1fr}.senior-quick button{font-size:16px;min-height:82px}}
</style>`;
...
function enhance(){
 ...
 const p=hero.querySelector('p'); if(p)p.innerHTML='메뉴를 찾지 않아도 됩니다.<br><b>하고 싶은 일을 그냥 말씀하세요.</b>';
 ...
 if(!quick){quick=document.createElement('div');quick.id='seniorQuick';quick.className='senior-quick';quick.innerHTML='<button class="voice-all" onclick="voiceAsk()">🎙 <b>그냥 말해서 부탁하기</b><small>무엇을 눌러야 할지 몰라도 괜찮아요</small></button><button onclick="photo(\'explain\')">📷 <b>이게 뭐야?</b><small>사진 찍어 물어보기</small></button><button onclick="photo(\'safe\')">🚨 <b>이거 괜찮아?</b><small>수상한 문자·안내 확인</small></button><button onclick="openTravel(\'route\')">📍 <b>어디로 가?</b><small>목적지와 길 찾기</small></button><button onclick="show(\'parking\')">🚗 <b>내 차 어디?</b><small>주차 위치 기억</small></button><button onclick="show(\'remember\')">🧠 <b>기억해줘</b><small>약속·병원·할 일</small></button><button onclick="quickAsk(\'휴대폰 사용법을 아주 쉽게 알려줘. 무엇이 어려운지 먼저 한 가지만 물어봐 줘.\')">📱 <b>휴대폰 도와줘</b><small>막힌 화면 쉽게 해결</small></button><button onclick="quickAsk(\'보낼 문자를 만들고 싶어. 누구에게 어떤 내용을 보낼지 하나씩 물어봐 줘.\')">💬 <b>문자 써줘</b><small>말하면 문장 만들어주기</small></button><button onclick="openNearby(\'병원 약국\')">🏥 <b>병원·약국</b><small>가까운 곳 찾기</small></button>';hero.insertAdjacentElement('afterend',quick)}
 if(!document.getElementById('seniorDaily')){const d=document.createElement('div');d.id='seniorDaily';d.className='senior-daily';d.innerHTML='<b>☀️ 오늘도 제가 옆에서 도와드릴게요.</b><br>사진, 길찾기, 기억, 휴대폰 사용까지 어려우면 먼저 <b>말하기</b>를 눌러보세요.';quick.insertAdjacentElement('afterend',d)}
 const title=byText('#home .sectionTitle','AI가 특히'); if(title)title.textContent='기억해 둔 것과 자주 쓰는 도움';
 ...
 const v=document.querySelector('#home .version'); if(v)v.textContent='시니어 AI 생활비서 · 실사용 중심판';
 if(!document.getElementById('seniorNote')){const n=document.createElement('div');n.id='seniorNote';n.className='senior-note';n.innerHTML='💡 <b>잘못 눌러도 괜찮아요.</b> 언제든 “⌂ 처음으로”를 누르면 돌아올 수 있습니다.';tools?.insertAdjacentElement('beforebegin',n)}
}
